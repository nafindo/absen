...
function doPost(e) {
  try {
    ...
    switch (action) {
      ...
      case 'getRecentErrors': return jsonResponse(getRecentErrors());
      ...
      case 'addAreaCeklist': return jsonResponse(addAreaCeklist(data));
      case 'getAreaCeklistAdmin': return jsonResponse(getAreaCeklistAdmin(data));
      case 'deleteAreaCeklist': return jsonResponse(deleteAreaCeklist(data));
      case 'getAreaCeklistKaryawan': return jsonResponse(getAreaCeklistKaryawan(data));
...
function uploadFotoToko(data) {
  try {
    ...
    try {
      ...
      console.error('Gagal set sharing:', err_sharing);
    }
...
function getRecentErrors() {
  try {
    const errorData = getSheetData(SHEET_NAMES.LOG_ERROR) || [];
    const now = new Date();
    // Ambil error dalam 48 jam terakhir
    const recent = errorData.filter(e => {
      const ts = parseTimestampSafe(e.Timestamp);
      return !isNaN(ts) && (now - ts) <= (48 * 3600000);
    });
    
    // Sort descending by Timestamp
    recent.sort((a, b) => {
      return parseTimestampSafe(b.Timestamp) - parseTimestampSafe(a.Timestamp);
    });
...
function checkUpdate() {
  try {
    ...
    for (let i = 0; i < settings.length; i++) {
      ...
      if (settings[i].Parameter === 'ADMIN_APK_VERSION_CODE') adminVersionCode = parseInt(settings[i].Value) || 7;
      if (settings[i].Parameter === 'ADMIN_APK_URL') adminApkUrl = String(settings[i].Value);
    }
    ...
    return {
      ...
      updateUrl: apkUrl,
      adminVersionCode: adminVersionCode,
      adminUpdateUrl: adminApkUrl
    };
  } catch (e) {
    return { success: false, error: e.toString() };
  }
...
function absenMasuk(data) {
  ...
  // Validasi Celah: Mencegah Karyawan curang pilih shift siang saat masuk pagi agar tidak telat
  const waktuShiftStart = new Date(now);
  waktuShiftStart.setHours(jamShift, menitShift, 0, 0);
  const bedaMenitAwal = (waktuShiftStart - now) / 60000;
  
  if (bedaMenitAwal > 150) { // Lebih dari 2.5 jam sebelum shift dimulai
    const todayStr = formatDate(now);
    const adaLembur = getSheetData(SHEET_NAMES.LEMBUR).some(l => 
        String(l.ID_Karyawan) === String(idKaryawan) && 
        (l.Tanggal instanceof Date ? formatDate(l.Tanggal) : String(l.Tanggal)).startsWith(todayStr) &&
        l.Status === 'Approved'
    );
    
    if (!adaLembur) {
       return { success: false, error: 'Terlalu awal untuk absen shift ini. Jika ini shift pagi, pastikan Anda memilih shift pagi. (Maksimal 2.5 jam sebelum shift tanpa lembur)' };
    }
  }
...
function absenPulang(data) {
  ...
  const now = new Date();
  let jamMasukReal = parseTimestampSafe(recordMasuk.Timestamp);
  if (isNaN(jamMasukReal.getTime())) {
    jamMasukReal = new Date(today + ' ' + String(recordMasuk.Jam_Masuk));
  }

  // Persiapan Variabel Jam
  const R_IN = jamMasukReal.getTime();
  const R_OUT = now.getTime();
  let S_IN = R_IN;
  let S_OUT = R_OUT;
  ...
  if (shift && shift.Jam_Masuk && shift.Jam_Pulang) {
    ...
    const sInStr = formatT(shift.Jam_Masuk).split(':').map(Number);
    const sOutStr = formatT(shift.Jam_Pulang).split(':').map(Number);
    
    if (sInStr.length >= 2 && sOutStr.length >= 2) {
      const shiftInDate = new Date(jamMasukReal);
      shiftInDate.setHours(sInStr[0], sInStr[1], 0, 0);
      S_IN = shiftInDate.getTime();
      
      const shiftOutDate = new Date(jamMasukReal);
      shiftOutDate.setHours(sOutStr[0], sOutStr[1], 0, 0);
      if (sOutStr[0] < sInStr[0]) {
        shiftOutDate.setDate(shiftOutDate.getDate() + 1); // Cross midnight
      }
      S_OUT = shiftOutDate.getTime();
    }
  }

  // Cek Status Lembur Approved
  const todayStr = formatDate(jamMasukReal);
  const lemburSheet = getSheet(SHEET_NAMES.LEMBUR);
  const lemburData = lemburSheet.getDataRange().getValues();
  let lemburRowIndex = -1;
  let adaLembur = false;
  
  for (let l = 1; l < lemburData.length; l++) {
    const lIdKar = String(lemburData[l][1]).trim();
    const lTgl = lemburData[l][5] instanceof Date ? formatDate(lemburData[l][5]) : String(lemburData[l][5]);
    const lStatus = String(lemburData[l][11]).trim();
    if (lIdKar === idKaryawan && lTgl.startsWith(todayStr) && lStatus === 'Approved') {
      adaLembur = true;
      lemburRowIndex = l + 1;
      break;
    }
  ...
  // LOGIKA JAM KERJA DAN LEMBUR (Berdasarkan Skenario yg Disetujui)
  let durasiMs = 0;
  ...
  if (isTelat) {
    // Skenario 1: Telat
    const effectiveOut = R_OUT > S_OUT ? S_OUT : R_OUT;
    durasiMs = effectiveOut - R_IN;
    
    // Lembur Akhir Shift (hanya jika Approved)
    if (adaLembur && R_OUT > S_OUT) {
      durasiLemburMs = R_OUT - S_OUT;
    }
  } else {
    // Skenario 2 & 3: Ontime / Lebih Awal
    if (isPulangAwal) {
      // Skenario 3: Pulang Awal
      durasiMs = R_OUT - S_IN; // Jam kerja dihitung dari Jam Shift Masuk
      
      // Lembur Awal (min 30 menit)
      if (adaLembur && (S_IN - R_IN) >= 30 * 60000) {
        durasiLemburMs = S_IN - R_IN;
      }
    } else {
      // Skenario 2: Kerja Penuh (Ontime / Lebih Awal, Pulang Tepat / Telat)
      durasiMs = S_OUT - S_IN;
      
      if (adaLembur) {
        // Lembur Awal (min 30 menit)
        if ((S_IN - R_IN) >= 30 * 60000) {
          durasiLemburMs += (S_IN - R_IN);
        }
        // Lembur Akhir
        if (R_OUT > S_OUT) {
  ...
  if (durasiLemburMs < 0) durasiLemburMs = 0;
  ...
  // Jika ada lembur yang disetujui, update durasinya secara presisi di sheet LEMBUR
  if (adaLembur && lemburRowIndex > 0) {
    const lemburJamVal = Math.floor(durasiLemburMs / 3600000);
    const lemburMenitVal = Math.floor((durasiLemburMs % 3600000) / 60000);
    // Konversi ke format decimal hours (contoh: 1.5) untuk Gaji
    const totalLemburDecimal = lemburJamVal + (lemburMenitVal / 60);
    
    lemburSheet.getRange(lemburRowIndex, 7).setValue(formatTime(new Date(R_IN))); // Jam Mulai Real
    lemburSheet.getRange(lemburRowIndex, 8).setValue(formatTime(now)); // Jam Selesai Real
    lemburSheet.getRange(lemburRowIndex, 9).setValue(totalLemburDecimal.toFixed(2)); // Durasi_Jam decimal
  }
...
function getJadwalHariIni(data) {
  ...
  const allJadwal = getSheetData(SHEET_NAMES.JADWAL_KARYAWAN);
  const validJadwalList = allJadwal.filter(j => {
  ...
  const jadwal = validJadwalList.length > 0 ? validJadwalList[validJadwalList.length - 1] : undefined;
...
function getJadwalMingguan(data) {
  ...
  for (let i = 0; i < 7; i++) {
    ...
    const validJadwalList = jadwalAll.filter(j => {
      if (!j.Hari_Berjalan.includes(namaHari)) return false;
      const tglMulaiDate = parseDateSafe(j.Tanggal_Mulai);
      const tglSelesaiDate = parseDateSafe(j.Tanggal_Selesai);
      const tglMulai = tglMulaiDate ? formatDate(tglMulaiDate) : '2000-01-01';
      const tglSelesai = tglSelesaiDate ? formatDate(tglSelesaiDate) : '2099-12-31';
      return tglStrForCompare >= tglMulai && tglStrForCompare <= tglSelesai;
    });

    const jadwal = validJadwalList.length > 0 ? validJadwalList[validJadwalList.length - 1] : null;
...
function getKaryawanJadwalByDate(data) {
  try {
    ...
    const validJadwalList = jadwalAll.filter(j => {
      ...
      const tglMulai = tglMulaiDate ? formatDate(tglMulaiDate) : '2000-01-01';
      const tglSelesai = tglSelesaiDate ? formatDate(tglSelesaiDate) : '2099-12-31';
      return targetDateStr >= tglMulai && targetDateStr <= tglSelesai;
    });
...
function formatRaport(absensi, mode, bln, thn, idKaryawan) {
  ...
  if (empId) {
    ...
    if (karyawanList) {
      ...
      if (karyawanObj) {
        gajiPokok = parseFloat(karyawanObj.Gaji_Pokok || 0);
        idTokoKaryawan = karyawanObj.ID_Toko || '';
      }
    }
  }

  // Hitung Tugas Luar Toko (Transport)
  let totalTugasLuarToko = 0;
  if (idTokoKaryawan) {
    absensi.filter(a => a.Tipe === 'Masuk').forEach(a => {
      const idTokoAbsen = a.ID_Toko || '';
      if (idTokoAbsen && String(idTokoAbsen) !== String(idTokoKaryawan)) {
        totalTugasLuarToko++;
      }
    });
  }

  const settings = getSettingGlobal().settings || {};
  const tarifTransport = parseFloat(settings.TARIF_UANG_TRANSPORT) || 15000;
  const uangTransport = totalTugasLuarToko * tarifTransport;
  ...
  return {
    ...
    totalIzin: totalIzin,
    totalCuti: totalCuti,
    totalTugasLuarToko: totalTugasLuarToko,
...
function saveToko(data) {
  ...
  appendRow(SHEET_NAMES.MASTER_TOKO, [
    idToko, nama, alamat, safeLat, safeLng, radius || 50, jamBuka || '08:00', jamTutup || '22:00', fotoUrl || '', 'Aktif', data.idSheetPos || ''
  ]);
...
function createTugas(data) {
  const { kategori, idToko, ditugaskanKe, judul, deskripsi, prioritas, dibuatOleh, deadline, modeToko, tunjanganRp } = data;
...
function getSalaries() {
  try {
    ...
    try {
      ...
      absensiData = getSheetData(SHEET_NAMES.ABSENSI) || [];
      settingData = getSheetData(SHEET_NAMES.SETTING_GLOBAL) || [];
      logTugasData = getSheetData(SHEET_NAMES.LOG_TUGAS) || [];
      tugasData = getSheetData(SHEET_NAMES.TUGAS) || [];
    } catch(e) {}
    ...
    const transportCountMap = {};
    let tarifTransport = 15000;
    const settingTarif = settingData.find(s => s.Parameter === 'TARIF_UANG_TRANSPORT');
    if (settingTarif && !isNaN(parseFloat(settingTarif.Value))) {
      tarifTransport = parseFloat(settingTarif.Value);
    }

    absensiData.forEach(a => {
      if (a.Tipe === 'Masuk' && a.Status_Masuk !== 'Tolak' && a.ID_Karyawan) {
        const tgl = parseDateSafe(a.Timestamp);
        if (tgl && (tgl.getMonth() + 1) === activeMonth && tgl.getFullYear() === activeYear) {
          const id = String(a.ID_Karyawan).trim().toLowerCase();
          const emp = list.find(k => String(k.ID_Karyawan).trim().toLowerCase() === id);
          if (emp) {
            const tokoDefault = String(emp.Toko_Default || '').trim().toLowerCase();
            const idTokoAbsen = String(a.ID_Toko || '').trim().toLowerCase();
            if (tokoDefault !== '' && idTokoAbsen !== '' && tokoDefault !== idTokoAbsen) {
              transportCountMap[id] = (transportCountMap[id] || 0) + 1;
            }
          }
        }
      }
    ...
    const tunjanganTugasMap = {};
    logTugasData.forEach(l => {
      const statusLog = String(l.Status_Verifikasi || l.Status || '').trim().toLowerCase();
      if ((statusLog === 'selesai' || statusLog === 'approved') && l.ID_Karyawan) {
        const tgl = parseDateSafe(l.Timestamp);
        if (tgl && (tgl.getMonth() + 1) === activeMonth && tgl.getFullYear() === activeYear) {
          const idK = String(l.ID_Karyawan).trim().toLowerCase();
          const taskObj = tugasData.find(t => t.ID_Tugas === l.ID_Tugas);
          if (taskObj && parseFloat(taskObj.Tunjangan_Rp) > 0) {
            tunjanganTugasMap[idK] = (tunjanganTugasMap[idK] || 0) + parseFloat(taskObj.Tunjangan_Rp);
          }
        }
      }
    });
    ...
    const data = filteredList.map(k => {
      ...
      const tCount = transportCountMap[idK] || 0;
      const calculatedTransport = tCount * tarifTransport;
      const calculatedTunjangan = tunjanganTugasMap[idK] || 0;
      ...
      if (!slip && isDraftPeriod) {
        ...
        headersGAJI.forEach(h => {
          ...
          else if (h === 'Uang_Transport') newRowValues.push(calculatedTransport);
          else if (h === 'Ket_Uang_Transport') newRowValues.push(tCount > 0 ? `${tCount}x luar toko` : '');
          else if (h === 'Tunjangan') newRowValues.push(calculatedTunjangan);
          else if (h === 'Ket_Tunjangan') newRowValues.push(calculatedTunjangan > 0 ? 'Tunjangan Tugas' : '');
          else if (['Bonus', 'Kasbon', 'Potongan_Lain', 'Total_Bersih'].includes(h)) newRowValues.push(0);
        ...
        slip = {
          ...
          Bonus: 0, Ket_Bonus: '', Uang_Transport: calculatedTransport, Ket_Uang_Transport: (tCount > 0 ? `${tCount}x luar toko` : ''),
          Tunjangan: calculatedTunjangan, Ket_Tunjangan: (calculatedTunjangan > 0 ? 'Tunjangan Tugas' : ''), Kasbon: 0, Ket_Kasbon: '',
...
function calculateAttendanceFromSheet(absensiSheet, employeeId, yearMonth, izinCutiSheet, tokoDefault) {
  ...
  for (var i = 1; i < absensiData.length; i++) {
    ...
    if (rowEmployeeId == employeeId && rowDateObj >= startDate && rowDateObj <= endDate) {
      ...
      var idTokoAbsen = row[3];
      
      var dateKey = Utilities.formatDate(rowDateObj, Session.getScriptTimeZone(), 'yyyy-MM-dd');
      presentDates[dateKey] = true;

      if (status !== 'TOLAK' && tokoDefault && idTokoAbsen && String(tokoDefault).trim().toLowerCase() !== String(idTokoAbsen).trim().toLowerCase()) {
         tugasLuarToko++;
      }
    ...
    for (var i = 1; i < scoresData.length; i++) {
      if (scoresData[i][3] == bulan && scoresData[i][10] == 'ACTIVE') {
        ...
        if (idKaryawan && idKaryawan == empId) {
          ...
          var attScore = calculateAttendanceFromSheet(absensiSheet, empId, bulan, null, tokoDefault);
          var taskScore = calculateTaskFromSheet(taskAssignmentsSheet, tasksSheet, empId, bulan);
          
          var settingSheet = ss.getSheetByName('SETTING_GLOBAL');
          var settingData = settingSheet ? settingSheet.getDataRange().getValues() : [];
          var tarifTransport = 15000;
          for (var s = 1; s < settingData.length; s++) {
             if (settingData[s][0] === 'TARIF_UANG_TRANSPORT' && !isNaN(parseFloat(settingData[s][1]))) {
                 tarifTransport = parseFloat(settingData[s][1]);
                 break;
             }
          }
          ...
          scorecard = {
            ...
            uangTransport: (attScore.tugas_luar_toko || 0) * tarifTransport,
            ketUangTransport: attScore.tugas_luar_toko > 0 ? (attScore.tugas_luar_toko + 'x luar toko') : '',
            detailKehadiran: { 
              ...
              izin: attScore.izin, 
              alpa: attScore.alpa, 
              tugasLuarToko: attScore.tugas_luar_toko || 0,
...
function submitChecklistHarian(data) {
  try {
    ...
    if (data.foto_base64 && data.foto_base64.length > 10) {
      photoUrl = uploadFotoToDrive(data.foto_base64, data.karyawan_id || id, "Checklist");
    }
    ...
    // --- CREATE TUGAS JIKA SKOR BURUK ---
    if (!data.kebersihan || !data.tata_letak || data.barang_rusak || data.barang_kosong) {
      const masalah = [];
      if (!data.kebersihan) masalah.push("Kotor");
      if (!data.tata_letak) masalah.push("Berantakan");
      if (data.barang_rusak) masalah.push("Ada barang rusak");
      if (data.barang_kosong) masalah.push("Ada stok kosong");
      
      const judulTugas = "[Perbaikan] " + (data.nama_rak || data.area_tugas || "Area");
      const deskripsiTugas = "Masalah ditemukan saat ceklist: " + masalah.join(", ") + ".\nCatatan: " + (data.catatan || "-");
    ...
    try {
      triggerPusher('pinguin-chat', 'ceklist-alert', {
        idKaryawan: data.karyawan_id || '',
        namaKaryawan: data.karyawan_nama || '',
        toko: data.toko_id || '',
        area: data.area_tugas || ''
      });
    } catch(pusherErr) {
      console.error("Gagal trigger pusher ceklist:", pusherErr);
    }
...
// ==================== AREA CEKLIST DINAMIS ====================

function addAreaCeklist(data) {
  try {
    const sheet = getSheet(SHEET_NAMES.MASTER_AREA_CEKLIST);
    const id = "AC_" + new Date().getTime();
    const row = [id, data.id_toko, data.id_karyawan, data.area_tugas, formatDateTime(new Date())];
    sheet.appendRow(row);
    return { success: true, message: "Area ceklist berhasil ditambahkan", id: id };
  } catch (error) {
    logError('addAreaCeklist', error, data);
    return { success: false, error: error.toString() };
  }
}

function getAreaCeklistAdmin(data) {
  try {
    const rows = getSheetData(SHEET_NAMES.MASTER_AREA_CEKLIST);
    return { success: true, data: rows };
  } catch (error) {
    logError('getAreaCeklistAdmin', error, data);
    return { success: false, error: error.toString() };
  }
}

function deleteAreaCeklist(data) {
  try {
    const success = deleteRow(SHEET_NAMES.MASTER_AREA_CEKLIST, "ID_Penugasan", data.id);
    if (success) {
      return { success: true, message: "Area ceklist berhasil dihapus" };
    } else {
      return { success: false, message: "Area ceklist tidak ditemukan" };
    }
  } catch (error) {
    logError('deleteAreaCeklist', error, data);
    return { success: false, error: error.toString() };
  }
}

function getAreaCeklistKaryawan(data) {
  try {
    const idKaryawan = data.id_karyawan;
    const idToko = data.id_toko;
    const rows = getSheetData(SHEET_NAMES.MASTER_AREA_CEKLIST);
    const filtered = rows.filter(r => r.ID_Karyawan === idKaryawan && r.ID_Toko === idToko);
    const areas = filtered.map(r => r.Area_Tugas);
    
    return { success: true, data: areas };
  } catch (error) {
    logError('getAreaCeklistKaryawan', error, data);
    return { success: false, error: error.toString() };
  }
}
